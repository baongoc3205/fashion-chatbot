...
function generateSKU(catSlug: string, index: number, size: string, color: string): string {
    ...
    return `${prefix}-${String(index).padStart(3, "0")}-${size}-${colorCode}`;
}

function generateDescription(name: string, brand: string, material: string, category: string): string {
    ...
    return descriptions[category] || `${name} ${brand} - ${material}. Sản phẩm chất lượng cao.`;
}
...
async function main() {
    ...
    const subCatMap: Record<string, { id: string; parent: string; gender: string }> = {};
    ...
    console.log(`   ✅ ${PARENT_CATEGORIES.length} parent + ${CATEGORIES.length} sub categories\n`);
    ...
    for (const [catSlug, templates] of Object.entries(PRODUCT_TEMPLATES)) {
        ...
        for (let i = 0; i < templates.length; i++) {
            ...
            const compareAtPrice = hasDiscount ? price + randomPrice(50000, 200000) : null;
            ...
            const numSizes = Math.min(sizes.length, 3 + Math.floor(Math.random() * 2));
            const shuffledSizes = [...sizes].sort(() => Math.random() - 0.5);
            const productSizes = shuffledSizes.slice(0, numSizes);

            const slug = slugify(name) + `-${brand.toLowerCase().replace(/\s/g, "")}-${i}`;
            ...
            const product = await prisma.product.create({
                data: {
                    ...
                    brand,
                    material,
                    careInstructions: "Giặt máy ở nhiệt độ thường. Không sử dụng chất tẩy. Phơi trong bóng râm.",
                    images: [`/images/products/${slug}-1.jpg`, `/images/products/${slug}-2.jpg`],
                    tags: [catSlug, brand.toLowerCase(), material.toLowerCase().split(" ")[0]],
                    ...
                    variants: {
                        create: productColors.flatMap((color) =>
                            productSizes.map((size) => ({
                                ...
                                color: color.name,
                                colorHex: color.hex,
                                sku: generateSKU(catSlug, productCount * 10 + i, size, color.name),
    ...
    console.log(`   📁 ${PARENT_CATEGORIES.length + CATEGORIES.length} categories`);
    console.log(`   📋 ${POLICIES.length} policies`);
